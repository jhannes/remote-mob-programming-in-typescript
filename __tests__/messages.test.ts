...
const english: IMessageTemplate = {
  ...
  serverError: (errorMessage) => "The server returned an error: " + errorMessage,
...
const norwegian: IMessageTemplate = {
  ...
  illegalArgument: (key, value) => "Verdien for '" + key + "' er ikke tillat: " + value,
};
...
describe("Messages", () => {
  it("Shows a message in english", ()=>{
    expect(
      showError(english,{code:"GeneralError"})
    ).toEqual("Something went wrong");
  }  
  );
  it("shows a message with an argument in english", ()=>{
    expect(
      showError(english,{code:"ServerError",errorMessage:"Database timeout"})
  ...
  it("Shows a message in Norwegian", ()=>{
    expect(
      showError(norwegian,{code:"GeneralError"})
    ).toEqual("Noe gikk galt");
  }  
  );
  it("shows a message with an argument in Norwegian", ()=>{
    expect(
      showError(norwegian,{code:"ServerError",errorMessage:"Database-tidsavbrudd"})
  ...
  it("Shows a formatted message witn multiple arguments in Norwegian", ()=>{
    expect(
      showError(norwegian,{code:"IllegalArgument",key:"foo",value:"bar"})
